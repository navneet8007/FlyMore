...
RSpec.describe Passenger, type: :model do
  ...
  describe "ActiveModel Validation" do
    ...
    it { expect(passenger).to allow_value("nav kr").for(:name) }
    it do
      expect(passenger).to allow_value("nav.kr@rubyastra.com").for(:email)
    end
    it do
      expect(passenger).not_to allow_value("nav.kr@rubyastra").for(:email)
    end
    it { expect(passenger).not_to allow_value("nav.kr").for(:email) }
