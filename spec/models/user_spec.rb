...
RSpec.describe User, type: :model do
  ...
  describe "ActiveModel validation" do
    ...
    it { expect(user).to allow_value("nav kr").for(:name) }
    it { expect(user).to allow_value("nav.kr@rubyastra.com").for(:email) }
    it { expect(user).not_to allow_value("nav.kr@rubyastra").for(:email) }
    it { expect(user).not_to allow_value("nav.kr").for(:email) }
